...
namespace DiscordBotD.other
{
    public class DrawingPrompt
    {
        ...
        private string[] colorScheme = { "Greyscale", "Triadic", "One Color Dominant", "High Key",
            "Low Key", "Vibrant", "Split complimentary", "Complementary", };

        private string[] anatomyStudy = { "Head", "Thorax", "Pelvis", "Arms", "Legs", "Hands", "Feet", "Hair", "Shoulders", "Abs" };
        ...
        public DrawingPrompt()
        {
            ...
            int themeIndex = random.Next(0, theme.Length - 1);
            int subjectIndex = random.Next(0, subject.Length - 1);
            int colorSchemeIndex = random.Next(0, colorScheme.Length - 1);
            int anatomyStudyIndex = random.Next(0, anatomyStudy.Length - 1);
            int cultureIndex = random.Next(0, culture.Length - 1);
            int universeIndex = random.Next(0, universe.Length - 1);
            int timeLimitIndex = random.Next(0, timeLimit.Length - 1);
